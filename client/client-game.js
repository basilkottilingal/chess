...
/*
.. ChessBoard is a class that maintains the divs regarding display of the board,
.. chess pieces, their drag properties, and other updates related to chess piece
.. moves
*/
export class ChessBoard
{
  ...
  /*
  .. can be used to load or reload chess board and the chess piece images
  */
  load (fen)
  {
    ...
    /* create an 8x8 grid using a for loop */
    ...
    /* Loop through each squares on the board */
    for (let i = 0; i < 8; i++)
    {
      for (let j = 0; j < 8; j++)
      {
        ...
        square.id = String.fromCharCode ('a'.charCodeAt(0) + j) + (8 - i);
      ...
      setTimeout (async () =>
      {
        ...
        if (this.piece)
        {
          ...
          if (this.move)
          {
            ...
            {
              /*
              .. In case of promotion, it has to wait till a promotion piece is
          ...
          {
            /* there is no valid move where the dragged piece ends up in this square */
            resolve (null);
          }
        }
        else
  ...
  squareDragOver (e)
  {
    let square = e.target.tagName === 'IMG' ? e.target.parentElement : e.target;
    if (this.piece)
    {
      if (square.classList.contains ('move-highlight'))
      {
        /*
        .. This makes sure that only the cells with 'move-highlight' (i.e possible
        .. target squares of the image/piece being dragged) can take part in 'dragover'
        .. (and thus 'drop'). You cannot drop a piece in squares which is not
        .. 'move-highlight'-ed
        */
        e.preventDefault ();
  ...
  /* Trigger when you drop a piece ('img') on this square ('div') */
  squareDrop (e)
  {
    let square = e.target.tagName === 'IMG' ? e.target.parentElement : e.target;
    if (this.piece)
    {
      ...
      /*
      .. Let chess.js know which move among moves is chosen by mouse drag/drop. 
      ...
      */
      ...
      if (found === false)
      {
        console.log('Error: Cannot find the move triggered by mouse drag');
      }
    }
  }

  /* list of eventListen related to drag and drop. The order in which these happen are 
  ..  dragstart → dragleave (if applicable) → drop → dragend.
  .. dragstart and dragend are associated with 'img' (like a chesspiece) while dragover,
  .. dragleave, drop are associated with 'div' (like a square on the chessboard).  
  */
  eventListen (colors)
  {
    return new  Promise ( (resolve) =>
    {
      /*
      .. It is very important to use a map, that maps each image to to it's dragstart
      .. or dragend event handle functions. Otherwise removing these eventListers are
      .. messy
      */
      ...
      pieces.forEach (piece =>
      {
        if (colors.includes (piece.id[0]))
        {
          ...
          const dragEndHandler = async (e) =>
          {
            ...
            /* delete all the eventListening of dragend/start even if move is null */
            pieces.forEach (img =>
            {
  ...
  startMove (move)
  {
    ...
    let piece = document.getElementById (move.from).querySelector ('img');
  ...
  {
    let piece = document.getElementById(move.to).querySelector('img');
  ...
  finishCastling (move)
  {
    /* King is already moved on the board. Now move the rook on the board*/
  ...
  {
    /* Remove opponent pawn from the board if the move is an en-passante one*/
    const square = document.getElementById (move.to[0] + move.from[1]);
  ...
  playerMove (move, isServer)
  {
    ...
    {
      /*
      .. Update player's move (mouse drag) into  the chess.js database and take
